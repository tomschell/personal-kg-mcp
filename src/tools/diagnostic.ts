...
import type { KnowledgeNode, KnowledgeEdge } from "../types/domain.js";
import { embedText, embedTextAsync, cosineSimilarity, isOpenAIAvailable } from "../utils/embeddings.js";
...
interface NodeWithScores extends KnowledgeNode {
...
function getImportanceWeight(node: KnowledgeNode): number {
  let weight = 1.0;
  if (node.importance === "high") weight = 3.0;
  else if (node.importance === "medium") weight = 1.5;
  else if (node.importance === "low") weight = 0.5;

  if (node.type === "decision") weight *= 2.0;
  else if (node.type === "question") weight *= 1.8;
  else if (node.type === "insight") weight *= 1.5;

  if (node.tags.includes("blocker")) weight *= 2.5;
...
function getTemporalWeight(node: KnowledgeNode, now: Date): number {
...
function getContextReason(node: KnowledgeNode, isIncluded: boolean, importanceWeight: number, temporalWeight: number): string {
  if (!isIncluded) {
    if (temporalWeight < 0.1) return "Temporal decay (old node)";
    if (importanceWeight < 1.0) return "Low importance weight";
    return "Below diversity threshold";
  }

  const reasons: string[] = [];
  if (node.type === "decision") reasons.push("Decision type (2x boost)");
  if (node.type === "question") reasons.push("Question type (1.8x boost)");
  if (node.type === "insight") reasons.push("Insight type (1.5x boost)");
  if (node.importance === "high") reasons.push("High importance (3x)");
  if (node.tags.includes("blocker")) reasons.push("Blocker tag (2.5x)");
  if (temporalWeight > 0.5) reasons.push("Recent (temporal boost)");

  return reasons.length > 0 ? reasons.join(", ") : "Diversity selection";
}

export function generateDiagnosticData(
  storage: FileStorage,
  project?: string
): DiagnosticData {
  const now = new Date();
  ...
  // Score all nodes
  const nodesWithScores: NodeWithScores[] = allNodes.map(node => {
...
export function generateHtmlDashboard(data: DiagnosticData): string {
  ...
  server.tool(
    ...
    async (args) => {
      ...
      try {
        ...
        const storagePath = (storage as any).baseDir || ".kg";
