...
const AnalyzeOperation = ["clusters", "emerging", "graph_export", "path"] as const;
...
async function handleEmerging(storage: FileStorage, limit: number, windowDays: number) {
...
async function handleGraphExport(storage: FileStorage) {
...
async function handlePath(storage: FileStorage, startId?: string, endId?: string, maxDepth: number = 4) {
  if (!startId || !endId) {
    return {
      content: [
        {
