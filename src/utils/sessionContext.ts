...
export interface ContextGroup {
  ...
  representativeNode?: KnowledgeNode;
}

export interface SmartContextResult {
  priorityNodes: KnowledgeNode[];  // High-importance, diverse nodes
  clusters: ContextGroup[];         // Grouped similar work
  summary: {
    totalNodes: number;
 ...
 */
function isHighVolumePattern(nodes: KnowledgeNode[], similarityThreshold = 0.75): boolean {
...
function calculateDiversityScore(
  ...
  if (selectedNodes.length === 0) return 1.0;

  const nodeVec = embedText(node.content);
  const similarities = selectedNodes.map(selected =>
    cosineSimilarity(nodeVec, embedText(selected.content))
  );
...
function getImportanceWeight(node: KnowledgeNode): number {
  ...
  else if (node.type === "progress") weight *= 1.0;
...
function groupSimilarNodes(
  ...
  for (const cluster of clusters) {
    ...
    let groupImportance: "high" | "medium" | "low" = "low";
...
export function selectSmartContext(
  ...
  const scoredNodes: ScoredNode[] = allNodes.map(node => {
  ...
  const workstreams = new Map<string, KnowledgeNode[]>();
  for (const node of allNodes) {
    const wsTag = node.tags.find(t => t.startsWith('ws:'));
    const key = wsTag || 'general';
    if (!workstreams.has(key)) workstreams.set(key, []);
    workstreams.get(key)!.push(node);
  }

  const highVolumePatterns: string[] = [];
  ...
  const highPriorityNodes = scoredNodes
    .filter(sn =>
      highPriorityTypes.has(sn.node.type) ||
  ...
  const candidatesWithDiversity: Array<{node: KnowledgeNode; score: number; diversity: number}> = [];
  ...
  if (selectedNodes.length < targetCount && allNodes.length > selectedNodes.length) {
    ...
    for (const node of remaining) {
      if (selectedNodes.length >= targetCount) break;
