...
export class MetricsStorage {
    ...
    recordToolCall(metric) {
        if (!this.config.enabled || !this.config.trackToolCalls)
            return;
     ...
     */
    recordSearch(metric) {
        if (!this.config.enabled || !this.config.trackSearches)
            return;
        const date = new Date(metric.timestamp).toISOString().split('T')[0];
     ...
     */
    recordNodeActivity(metric) {
        if (!this.config.enabled || !this.config.trackNodeActivity)
            return;
        const date = new Date(metric.timestamp).toISOString().split('T')[0];
     ...
     */
    getToolCalls(startDate, endDate) {
        const metrics = [];
        ...
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
            ...
            const file = join(this.baseDir, 'tool-calls', `${date}.jsonl`);
            metrics.push(...this.readJSONL(file));
        }
        return metrics;
    }
    /**
     ...
     */
    getNodeActivity(startDate, endDate) {
        const metrics = [];
        const start = new Date(startDate);
        const end = new Date(endDate);
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
     ...
     */
    getEmbeddings(startDate, endDate) {
        const metrics = [];
        const start = new Date(startDate);
        const end = new Date(endDate);
        for (let d = new Date(start); d <= end; d.setDate(d.getDate() + 1)) {
     ...
     */
    generateSnapshot(period, totalActiveNodes) {
        const now = new Date();
        let startDate;
        switch (period) {
            case '1h':
     ...
     */
    getTimeSeries(type, startDate, endDate, interval = 'hour') {
        let data = [];
        switch (type) {
            case 'tool-calls':
