...
const EdgeOperation = ["create", "list", "maintain"];
const RelationType = ["references", "relates_to", "derived_from", "blocks", "duplicates"];
const MaintenanceOp = ["rebuild", "prune", "reclassify", "comprehensive"];
export function setupRelationshipTools(server, storage) {
    ...
    // =============================================================================
    server.tool("kg_edges", "Unified tool for relationship operations. Supports: 'create' to link nodes, 'list' to view relationships, 'maintain' for cleanup and rebuilding.", {
        ...
        const { operation, fromNodeId, toNodeId, relation, nodeId, maintainOp = "comprehensive", rebuildThreshold = 0.35, pruneThreshold = 0.15, limit = 1000 } = args;
...
// =============================================================================
async function handleCreateEdge(storage, fromNodeId, toNodeId, relation) {
...
async function handleListEdges(storage, nodeId) {
...
async function handleMaintenance(storage, maintainOp, rebuildThreshold, pruneThreshold, limit) {
        ...
        for (let i = 0; i < nodes.length; i++) {
            for (let j = i + 1; j < nodes.length; j++) {
                ...
                if (strength >= rebuildThreshold) {
                    ...
                    const hasRelationship = existingEdges.some(edge => (edge.fromNodeId === nodeA.id && edge.toNodeId === nodeB.id) ||
