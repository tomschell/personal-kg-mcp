...
export function setupMaintenanceTools(server, storage) {
    ...
    // =============================================================================
    server.tool("kg_admin", "Unified admin tool for maintenance operations. Supports: 'health' for system status, 'backup' for data protection, 'validate' for integrity checks, 'repair' for fixing issues, 'export'/'import' for data migration, 'rename_tag'/'merge_tags' for tag management, 'migrate_embeddings' for backfilling OpenAI embeddings.", {
...
async function handleBackup(storage, retentionDays) {
...
async function handleValidate(storage) {
...
async function handleRepair(storage) {
...
async function handleExport(storage) {
...
async function handleImport(storage, payload) {
    if (!payload) {
        return {
            content: [
                {
...
async function handleRenameTag(storage, oldTag, newTag, dryRun = false) {
    if (!oldTag || !newTag) {
        return {
            content: [
                {
...
async function handleMergeTags(storage, sourceTags, targetTag, dryRun = false) {
    ...
    const nodes = storage.listAllNodes();
    const affected = nodes.filter((n) => n.tags.some((t) => sourceTags.includes(t)));
    if (!dryRun) {
        for (const node of affected) {
...
async function handleMigrateEmbeddings(storage, batchSize, force) {
