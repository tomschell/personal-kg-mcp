...
// Search mode type
const SearchMode = ["text", "semantic", "time_range"];
...
USE_ANN, EMBED_DIM, tagCo // TODO: Type this properly
    ...
    // =============================================================================
    server.tool("kg_search", "Unified search tool for finding knowledge nodes. Supports three modes: 'text' for keyword/tag filtering with ranking, 'semantic' for AI-powered meaning-based search, 'time_range' for date-based queries. Default mode is 'semantic' for best results.", {
        // Core parameters
    ...
    // =============================================================================
    server.tool("kg_list_tags", "Lists all tags in the knowledge graph with usage counts. Use to discover available tags, find inconsistencies, or identify commonly used categories.", {
...
async function handleTextSearch(storage, EMBED_DIM, args) {
    ...
    const fmt = { format, includeContent, includeTags, includeMetadata };
    const payload = { mode: "text", total: nodes.length, nodes: formatNodes(nodes, fmt) };
    return {
        content: [
            {
                type: "text",
...
async function handleSemanticSearch(storage, ann, USE_ANN, EMBED_DIM, args) {
...
async function handleTimeRangeSearch(storage, args) {
...
// =============================================================================
export function setupContextTools(server, storage, EMBED_DIM) {
    server.tool("kg_get_relevant_context", "Retrieves relevant past context for a given query. Returns brief summaries of related decisions, insights, and open questions. Perfect for proactive context injection before starting work on a topic.", {
        query: z.string()
        ...
        if (include_questions) {
            const questionNodes = scored.filter((s) => s.node.type === "question" && s.similarity > 0.15);
