    ...
    for (const [branch, entries] of Object.entries(branchGroups)) {
        ...
        for (const entry of entries) {
            ...
            if (entry.tags.length > 0) {
                const relevantTags = entry.tags.filter(tag => tag.startsWith("proj:") || tag.startsWith("ws:") || tag.startsWith("issue:"));
 ...
 */
export function formatBranchActivity(activities, options) {
 ...
 */
function formatMetrics(metrics, timeWindow) {
    return `## Metrics\n\n` +
 ...
 */
function groupByBranch(timeline) {
    const groups = {};
    for (const entry of timeline) {
