 ...
 */
export function extractBranchFromTags(tags) {
 ...
 */
export function getTimeAgoMinutes(timestamp) {
    const now = new Date();
 ...
 */
export function detectContextSwitches(timeline) {
 ...
 */
function isCompletedWork(entry) {
    // Check for completion indicators in tags
    const completionTags = ["completed", "done", "finished", "resolved", "closed"];
    const hasCompletionTag = entry.tags.some(tag => completionTags.some(completionTag => tag.toLowerCase().includes(completionTag)));
    ...
    const hasIssueCompletion = entry.tags.some(tag => tag.startsWith("issue:") && (tag.includes("closed") ||
 ...
 */
export function groupNodesByBranch(nodes) {
 ...
 */
export function aggregateBranchActivity(timeline) {
    const branchMap = new Map();
