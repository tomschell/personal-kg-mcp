...
function extractCommitTags(node) {
    const tags = Array.isArray(node.tags) ? node.tags : [];
    const inTags = [];
    for (const t of tags) {
        const m = t.match(/^commit:([0-9a-f]{7,40})$/i);
        if (m)
            inTags.push(m[1].toLowerCase());
    }
    const inContent = [];
    const re = /\bcommit:([0-9a-f]{7,40})\b/gi;
    let m;
    while ((m = re.exec(node.content)))
        inContent.push(m[1].toLowerCase());
    return [...new Set([...inTags, ...inContent])];
}
export function computeStrengthFactors(a, b) {
    // Content similarity
    const contentSimilarity = clamp01(cosineSimilarity(embedText(a.content), embedText(b.content)));
    ...
    if (sharedCommit)
        explicitReferences = Math.max(explicitReferences, 1);
...
export function scoreRelationship(a, b) {
    const f = computeStrengthFactors(a, b);
    const weighted = f.contentSimilarity * StrengthWeights.contentSimilarity +
...
export function classifyRelationship(a, b) {
