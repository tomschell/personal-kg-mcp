...
export function createPersonalKgServer() {
    ...
    server.tool("kg_get_node", "Retrieves a specific knowledge node by its unique ID. Use to fetch detailed information about a particular node including its content, metadata, tags, and relationships.", { id: z.string()
            .describe("Unique identifier of the knowledge node to retrieve") }, async ({ id }) => {
    ...
    server.tool("kg_list_recent_summary", { limit: z.number().int().min(1).max(100).default(20), summaryLength: z.number().int().min(1).max(2000).optional() }, async ({ limit, summaryLength }) => {
        warnDeprecated("kg_list_recent_summary", "Use kg_list_recent with format=\"summary\" and optional summaryLength instead; this convenience tool will be removed in a future release.");
        logToolCall("kg_list_recent_summary", { limit, summaryLength });
        const nodes = storage.listRecent(limit);
        const payload = { total: nodes.length, nodes: formatNodes(nodes, { format: "summary", summaryLength }) };
        return { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }], structuredContent: payload };
    });
    server.tool("kg_list_recent_minimal", { limit: z.number().int().min(1).max(100).default(20) }, async ({ limit }) => {
        warnDeprecated("kg_list_recent_minimal", "Use kg_list_recent with format=\"minimal\" instead; this convenience tool will be removed in a future release.");
        logToolCall("kg_list_recent_minimal", { limit });
        const nodes = storage.listRecent(limit);
        const payload = { total: nodes.length, nodes: formatNodes(nodes, { format: "minimal" }) };
        return { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }], structuredContent: payload };
    });
    ...
    server.tool("kg_mark_blocks", { sourceId: z.string(), targetId: z.string() }, async ({ sourceId, targetId }) => {
        warnDeprecated("kg_mark_blocks", "Use kg_create_edge with relation=\"blocks\" instead; this wrapper will be removed in a future release.");
    ...
    server.tool("kg_mark_blocked_by", { targetId: z.string(), blockerId: z.string() }, async ({ targetId, blockerId }) => {
        warnDeprecated("kg_mark_blocked_by", "Use kg_create_edge with relation=\"blocks\" instead; this wrapper will be removed in a future release.");
    ...
    server.tool("kg_mark_affects", { sourceId: z.string(), targetId: z.string() }, async ({ sourceId, targetId }) => {
        warnDeprecated("kg_mark_affects", "Use kg_create_edge with relation=\"references\" instead; this wrapper will be removed in a future release.");
    ...
    server.tool("kg_prune_weak_relationships", { threshold: z.number().min(0).max(1).default(0.15) }, async ({ threshold }) => {
        warnDeprecated("kg_prune_weak_relationships", "Use kg_relationships_maintenance with pruneThreshold instead; this tool will be removed in a future release.");
    ...
    server.tool("kg_reclassify_relationships", { limit: z.number().int().min(1).max(10000).default(2000) }, async ({ limit }) => {
        warnDeprecated("kg_reclassify_relationships", "Use kg_relationships_maintenance (rebuilds + prunes) or a future reclassify mode; this tool will be removed in a future release.");
    ...
    server.tool("kg_search_summary", {
        ...
        warnDeprecated("kg_search_summary", "Use kg_search with format=\"summary\" and optional summaryLength instead; this convenience tool will be removed in a future release.");
    ...
    server.tool("kg_search_minimal", {
        query: z.string().optional(),
        tags: z.array(z.string()).optional(),
        type: z.enum(KnowledgeNodeType).optional(),
        limit: z.number().int().min(1).max(100).default(20),
    }, async ({ query, tags, type, limit }) => {
        warnDeprecated("kg_search_minimal", "Use kg_search with format=\"minimal\" instead; this convenience tool will be removed in a future release.");
    ...
    function generateAttentionAlerts(dashboardResult) {
        const alerts = [];
        ...
        if (metrics.contextSwitches > 5) {
            alerts.push({
                ...
                affectedBranches: [...new Set(timeline.map((t) => t.branch))],
                timeframe: 'recent'
            });
        }
        // Focus drift alert
        ...
        const activeBranches = new Set(timeline.map((t) => t.branch));
    ...
    server.tool("kg_session_warmup", "Start every session with this tool! Loads comprehensive context about your project including recent work, active questions, blockers, and dashboard metrics. Essential for maintaining continuity between work sessions.", {
        project: z.string()
    ...
    server.tool("kg_query_code", "Searches for knowledge related to specific code files or functions. Use to find decisions, insights, or progress related to particular code components.", {
        filePath: z.string()
    ...
    server.tool("kg_find_connection_path", "Finds the shortest path between two knowledge nodes through their relationships. Use to discover how different ideas or work items are connected in the knowledge graph.", {
        startId: z.string()
    ...
    server.tool("kg_graph_export", "Exports the complete knowledge graph structure for external analysis or visualization. Use to create backups, share knowledge graphs, or perform external analysis.", {}, async () => {
        logToolCall("kg_graph_export");
        const g = buildGraphExport(storage);
        return { content: [{ type: "text", text: JSON.stringify(g, null, 2) }] };
    });
    server.tool("kg_detect_topic_clusters", "Identifies groups of related knowledge nodes using similarity analysis. Use to discover themes, workstreams, or conceptual clusters in your knowledge base.", {
    ...
    server.tool("kg_find_emerging_concepts", "Identifies new or emerging topics based on recent activity patterns. Use to spot trends, new work areas, or evolving concepts in your knowledge base.", {
        limit: z.number().int().min(1).max(10000).default(500)
    ...
    server.tool("kg_workstream_dashboard", {
